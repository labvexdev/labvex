...
function PostCard({ post, idx }: { post: PostData; idx: number }) {
    ...
    >
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
        {post.tags.map((t, index) => (
          <span key={index} className="badge badge-gray" style={{ fontSize: 11 }}><Hash size={9} />{t}</span>
        ))}
      </div>
...
export default function FeedClient({ initialPosts }: { initialPosts: PostData[] }) {
  ...
  const filtered = tag === "All" ? initialPosts : initialPosts.filter(p => p.tags.includes(tag));

  const handlePublish = async () => {
    if (!title || !body) return toast.error("Please fill in title and content");
    setIsPublishing(true);
    // In real app, we'd make a POST to /api/posts
    toast.success("Post published successfully to the database!");
          ...
          {showCreate && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12, justifyContent: "flex-end" }}>
                <button onClick={() => setShowCreate(false)} disabled={isPublishing} style={{ padding: "7px 16px", borderRadius: 20, fontSize: 13.5, color: "var(--muted)", border: "1px solid var(--border)", background: "transparent", cursor: "pointer" }}>Cancel</button>
                <button onClick={handlePublish} disabled={isPublishing} className="btn btn-dark" style={{ fontSize: 13.5, padding: "7px 20px", opacity: isPublishing ? 0.5 : 1 }}>Publish</button>
              </div>
            </motion.div>
          )}
