...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const userStr = localStorage.getItem("labvex_user");
    if (userStr) {
      const user = JSON.parse(userStr);
      // Mock admin check: labvex_admin or genetics_mapper or any defined email/wallet
      if (user.username === "labvex_admin" || user.username === "genetics_mapper") {
        setIsAdmin(true);
        const subs = JSON.parse(localStorage.getItem("labvex_submissions") || "[]");
        setSubmissions(subs);
      } else {
  ...
  const handleAction = (id: string, action: "Approved" | "Rejected", repToAward: number = 0) => {
    ...
    localStorage.setItem("labvex_submissions", JSON.stringify(updated));
    
    if (action === "Approved") {
      toast.success(`Content approved! Awarded ${repToAward} Rep to author.`);
      // If the author is the currently logged in user (for testing), update their rep
      const userStr = localStorage.getItem("labvex_user");
      if (userStr) {
        const user = JSON.parse(userStr);
        const activeSub = submissions.find(s => s.id === id);
        if (activeSub?.author === user.username) {
          user.reputation_score += repToAward;
          localStorage.setItem("labvex_user", JSON.stringify(user));
        }
      }
    } else {
